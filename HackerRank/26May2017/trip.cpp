...
bool check(int a, int b, ll payy, ll gass, int topp, deque<int> &dq) {
    ...
    bool same = true;
    for (int i=0; i<dq.size() && i<minns.size(); i++) {
        same = same && dq[i] == minns[i];
    }
    cerr << endl;

    bool ret = same && payed == payy && gas == gass && dq.size() == minns.size()
        && topp>=a && topp<b;
...
int main() {
    ...
    for (int ii = 0; ii<q; ii++) {
        ...
        if ( sec != prevSec) {
            ...
            if ( q.first != 0) {
                dq.push_back(a);
                toppestLowInQueue = true;
            }
            else toppestLowInQueue = false;
        ...
        while(a>aa) {
            ...
            if (!dq.empty()) {
                ...
                while(prev_best >= P[a] && prev_best_i != b+1) {
                    ...
                    if (dq.front() == toppestLow) {
                        toppestLowInQueue = false;
                    }
                    dq.pop_front();
        ...
        while(b<bb) {
            ...
            //cerr << toppestLow << " " << P[toppestLow] << "toppestLow and price\n";
            if (gas < W[b]) {
                ll needGas = W[b]-gas;
                gas += needGas;
                payed += P[toppestLow]*needGas;
                if ( !toppestLowInQueue) {
                    dq.push_back(toppestLow);
                    toppestLowInQueue = true;
                }
            }
            ...
            if ( dq.empty() ) {
                ...
                if ( b != i )  {
                    ...
                    if (dq.size() > 1) {
                        ...
                        int j = dq.back();
                        dq.push_back(i);
                        pll pq = seg.query(j, i-1);
                        prevGas = pq.second;
                    }
                    else {
                        pll pq = seg.query(a, i-1);
                        prevGas = pq.second;
                    }

                    payed -= P[i] * (q.first - prevGas);

                    //cerr << "q " << q.first << " " << q.second << endl;
                    //cerr << "qq " << qq.first << " " << qq.second << endl;
                    //cerr << "prevGas " << prevGas << endl;

                    if ( qq.first > prevGas) {
                        //cerr << "if?????????????\n";
                        payed += P[i] * (qq.first - prevGas);
                        gas = qq.second;
                        if ( toppestLow == b ) {
                            toppestLow = i;
                            toppestLowInQueue = true;
                        ...
                        toppestLow = i;
                        toppestLowInQueue = false;
                        gas = qq.second + (prevGas - qq.first);
                    }
                }
                else {
                    ll prevGas;
        ...
        //cerr << "And the anser for " << aa << " " << bb << " is : " << payed << "\n";
