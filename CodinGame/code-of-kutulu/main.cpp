...
bool straitBetween(pii a, pii b) {
  if (a.first != b.first && a.second != b.second) return false;
  if (a == b) return true;
  pii diff = b-a;
  int length = abs(diff.first + diff.second);
  pii step = diff/length;
  while(a != b) {
    if (IND(grid, a) == '#') return false;
    a = a+step;
  }
  return true;
}

bool straitBetweenThroughWalls(pii a, pii b) {
  return (a.first == b.first || a.second == b.second);
}


template<typename F>
pii bfs(pii start, int maxDepth, F f) {
  ZERO_OUT(vis);
  queue<pii> q;
  IND(vis, start) = true;
  q.push(start);
    ...
    for (Entity &entity : entities) {
      if (entity.entityType == EXPLORER && entity.id != playerId) {
        ...
        if (entity.param1 == MINION_STUNNED) {
          ...
          IND(orig_spook, entity.position) -= sanityLossSpook * discouts[1];
        } else {
          ...
          vector<Player> relevant;
          bool targetAvailable = false;
          pii targetLocation(-1,-1);

          for (Player &player: otherPlayers) {
            if (straitBetween(player.position, entity.position)) {
              pii ep = entity.position;
              if (bfs(player.position, tillRush+1, [ep](pii p, int _) {
                    return straitBetweenThroughWalls(p, ep);
                    }).first == -1) {
                relevant.push_back(player);
                if (player.id == idRushing) {
          ...
          if (targetAvailable) {
            bfs(targetLocation, tillRush+1, [value](pii p, int _) {
                IND(reward, p) -= value;
                return true;
                });
          }else {
            ...
            if (idRushing != playerId) {
              for (Player &player: relevant) {
                minDistance = min (minDistance, manhattan(player.position, entity.position));
              }
            }
            ...
            for (pii dir : walkCells) {
              ...
              while (IND(grid, pp) != '#') {
                ...
                IND(reward, pp) -= value;
                if (!IND(hasPlayer,pp) && cnt == minDistance) break;
                if (cnt == minDistance+1) break;
                pp = pp + dir;
                cnt ++;
              }
            }
